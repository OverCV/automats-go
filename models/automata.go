...
func (a *Automata) NewState(data string, initial bool, final bool) {
...
func (a *Automata) NewTransition(start string, end string, data []string) error {
	newTransition := NewTransition(start, end, data)

	if a.ExistObj(newTransition, a.Transitions) {
		return fmt.Errorf("la transición ya existe")
	}
	...
	if a.ExistState(start) && a.ExistState(end) {
		...
		fmt.Print("Transición creada exitosamente.")
		return nil
	}
	return fmt.Errorf("no se agregó la transición")
}
